...
import ru.ibra.bot.config.CurrencyRateClientConfig;
...
public class CurrencyRateClientImpl implements CurrencyRateClient {
    ...
    public CurrencyRate getCurrencyRate(String rateType, String currency, LocalDate date) {
        ...
        String urlWithParams = String.format("%s/%s/%s/%s", config.getUrl(), rateType, currency, DATE_FORMATTER.format(date));
